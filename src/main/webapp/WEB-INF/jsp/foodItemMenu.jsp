...
<script>
	...
	function getActiveCategory() {
		...
		var data = "";
				...
				$.ajax({
						type : "GET",
						url :"CategoryMenu/getActiveCategory/"+ status,
							 ...
							 data = response
								...
								for (i = 0; i < data.length; i++) {
										...
										$("#categoryRow").append(
												...
												+ '<input type="button" class="add-to-cart btn btn-primary" onclick="getActiveFoodItem('+ data[i].id +')" value="View Category"></input>'
												+ '</div> </div>');
									}
	...
	function getActiveFoodItem(id){
		...
		$.ajax({
			type : "POST",
			url : "FoodItemMenu/getActiveFoodItem",
			dataType : 'json',
			data : {
	...
	function foodItemCartBTN(id) {
		...
		$.ajax({
			type : "GET",
			url : "FoodItemMenu/foodItemCart/" + id,
			dataType : 'json',
